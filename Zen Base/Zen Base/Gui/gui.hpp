...
// I dont recommend messing with this.
...
namespace gui
{
	...
	bool init()
	{
		...
		create_window_in_band = reinterpret_cast<CreateWindowInBand>(GetProcAddress(input::user32, "CreateWindowInBand"));
		if (!create_window_in_band)
		{
			create_window_in_band = reinterpret_cast<CreateWindowInBand>(GetProcAddress(input::win32u, "CreateWindowInBand"));
